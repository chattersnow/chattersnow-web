-- Retention clears photo and media consent with the name (#599)
-- ---------------------------------------------------------------------------
--
-- 20260922070000 added `photo_consent`, `photo_consent_at` and
-- `photo_consent_text` to `event_registrations`, and the purge's rule C does
-- not know about them.
--
-- THIS IS THE OPPOSITE CALL FROM THE WAIVER'S, deliberately, and the reason is
-- the whole content of this migration.
--
-- #686 kept `waiver_accepted_at` and `waiver_version` through anonymization,
-- beside `party_size` and `checked_in_at`, because an acceptance is a fact
-- about an ACT: this person agreed to these terms on this date. That sentence
-- is still true and still meaningful once the name has gone, which is why
-- `docs/legal-basis.md` notes the awkward consequence -- a row reading "Removed
-- accepted waiver v2" -- as a retention question rather than an error.
--
-- Photo consent is a fact about a person's FACE, and it does not survive the
-- same treatment. An anonymized "declined" protects nobody: the record exists
-- so that somebody holding a camera can check a name against a list, and there
-- is no name left to check. An anonymized "granted" authorizes nothing, for
-- the same reason. What would remain is a three-year-old permission attached to
-- a row nobody can identify, which is a liability rather than a record. So all
-- three columns are nulled beside `name`, `email` and `person_id`.
--
-- The snapshot goes with them. On its own it is the organization's own
-- published text and would be harmless to keep -- that is the argument
-- 20260921000000 makes for `consented_terms`, which is never redacted -- but it
-- is only here to say what a particular person agreed to, and the pair
-- constraint would have to be loosened to keep it beside a null answer. Keeping
-- a copy of the organization's own words against an anonymous row buys nothing.
--
-- Two halves, and the second is the one that gets skipped: `photo_consent is
-- not null` joins rule C's SELECT as well as its UPDATE, so a row whose only
-- remaining personal data is a consent answer is still picked up on a later
-- run.
--
-- `audited_tables` still has no row for event_registrations (20260922000000
-- says why), so `redacted_columns` does not arise.
--
-- The body is the live one from 20260922050000 -- which added the four
-- accompanying-adult and emergency contacts to rule C (#685) -- with those two
-- edits and nothing else. Re-emitted in full because the function is one
-- plpgsql block, so changing a rule is a rewrite rather than a patch; the
