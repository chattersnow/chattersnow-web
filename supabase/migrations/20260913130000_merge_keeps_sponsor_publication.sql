-- #1030: a merge must not silently unpublish a sponsor.
--
-- #1024 put the public sponsor wall's second opt-in on
-- `person_role_tags.is_public` (20260913070000): an organization with a
-- hand-asserted sponsor tag marked public appears on /support/sponsorship with
-- no event anywhere. That turned the tag row from a bare assertion into one
-- carrying a value.
--
-- merge_people still treated it as the former. Before its generic repoint it
-- deletes the duplicate's tags wherever the survivor already holds the same
-- role -- correct while the row said nothing but "this person is a sponsor",
-- and wrong once it also says "and the public can see them". Merging a
-- published record into an unpublished duplicate of the same organization,
-- which is exactly what the merge tool is for and exactly the record that gets
-- entered twice, dropped the flag and took the logo off the public site with
-- nothing said to the staffer.
--
-- The fix is to carry the flag onto the survivor before the delete. It is
-- three lines inside the function, but `create or replace function` has no
-- smaller unit than the whole body, so the rest of merge_people is restated
-- here byte for byte from 20260906080000 -- the only changes are the update
