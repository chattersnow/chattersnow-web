...
- **Environment variables** (`.env.local`, not committed): `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `NEXT_PUBLIC_SITE_URL` (public site origin, used to build links from Server Actions where there's no `window.location`), `NEXT_PUBLIC_PORTAL_URL` (portal origin, e.g. `https://portal.chattersnow.org` — invite/confirm links must land on the portal host so the session cookie is set there; falls back to `NEXT_PUBLIC_SITE_URL` for local dev and previews, where portal and site share one host), and `SUPABASE_SECRET_KEY` (service-role key, for trusted server-only operations — never reaches browser code; only read in `src/lib/supabase/admin.ts`, which is `server-only`-guarded and must only be imported from server components/route handlers/Server Actions, never from `src/lib/supabase/client.ts` or anything marked `"use client"`).
