...
Specs that mutate state the whole site shares can't run beside anything else, so they get a Playwright project of their own that depends on every browser project and therefore runs last and alone — currently just `page-visibility.spec.ts`, which toggles a real `page_visibility.*` row in `app_settings` (#594). Narrow the browser matrix with the `E2E_BROWSERS` env var (`E2E_BROWSERS=chromium bun run test:e2e`) rather than `--project`, so that dependency edge keeps covering exactly the projects that are running. Playwright runs a dependency project in full regardless of any file or `--grep` filter, so pass `--no-deps` when running one of those specs on its own: `bunx playwright test e2e/page-visibility.spec.ts --no-deps`. Filters that don't select that project are unaffected. `--shard` is one more filter a dependency project ignores, which is why the CI run is split rather than simply sharded (#753): `E2E_MUTATING=skip` builds only the browser projects (nothing is a dependency, so `--shard` works) and `E2E_MUTATING=only` builds only `page-visibility` with no dependency edge. `.github/workflows/e2e.yml` runs those as separate jobs on separate Supabase stacks and merges the blob reports; leaving `E2E_MUTATING` unset — what `bun run test:e2e` and `test:e2e:pr` do — keeps the single ordered local run.
