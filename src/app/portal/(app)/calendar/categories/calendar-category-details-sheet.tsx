...
function formStateFor(category: CalendarCategoryRow): FormState {
  return {
...
function isDirty(form: FormState, category: CalendarCategoryRow) {
...
export function CalendarCategoryDetailsSheet({
  category,
  itemCount,
  canManage,
}: {
  category: CalendarCategoryRow;
  /** How many calendar items carry this category, so the delete can explain itself. */
  itemCount: number;
  canManage: boolean;
}) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await updateCalendarCategoryAction(category.id, formData);
      if ("error" in result) {
  ...
  function handleDelete() {
    setError(null);
    startTransition(async () => {
      const result = await deleteCalendarCategoryAction(category.id);
        ...
        <SheetContent side="right" showCloseButton={false}>
          <SheetHeader className="flex-row items-start gap-2 space-y-0">
            ...
            <div className="flex flex-1 flex-col gap-0.5">
              <SheetTitle>
                {mode === "edit" ? "Edit category" : "Calendar category"}
              </SheetTitle>
              <SheetDescription>
                {mode === "edit"
                  ? "Rename it, reorder it, or retire it."
