...
describe("parseCalendarItemForm", () => {
  test("parses valid input", () => {
    ...
    expect("data" in result && result.data.title).toBe("Transgender Day of Visibility");
    expect("data" in result && result.data.priorityTier).toBe(1);
    expect("data" in result && result.data.decision).toBeNull();
  });

  test("requires a title", () => {
    expect(parseCalendarItemForm(formData({ ...validFields, title: "" }))).toEqual({
      error: "Title is required.",
    });
  });

  test("requires a valid item type", () => {
    expect(parseCalendarItemForm(formData({ ...validFields, itemType: "made_up" }))).toEqual({
  ...
  test("requires an owner once status moves past idea", () => {
    const result = parseCalendarItemForm(formData({ ...validFields, calendarStatus: "active" }));
    expect(result).toEqual({
      error: "An owner is required once a calendar item moves past idea status.",
    });
  });
  ...
  test("accepts an owner for active status", () => {
    const result = parseCalendarItemForm(
      formData({ ...validFields, calendarStatus: "active", ownerId: "11111111-1111-1111-1111-111111111111" })
    );
    expect("data" in result && result.data.calendarStatus).toBe("active");
  });

  test("requires a reason when an item is skipped", () => {
    const result = parseCalendarItemForm(formData({ ...validFields, decision: "skip" }));
    expect(result).toEqual({ error: "A reason is required when an item is skipped." });
  });

  test("accepts a skip decision with a note", () => {
    const result = parseCalendarItemForm(
      formData({ ...validFields, decision: "skip", decisionNote: "Handled offline this year." })
    );
  ...
  test("rejects an invalid category", () => {
    const result = parseCalendarItemForm(formData({ ...validFields, categories: ["not_real"] }));
