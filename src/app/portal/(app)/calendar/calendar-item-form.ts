...
export function parseCalendarItemForm(
  ...
  const priorityRationale = String(
    formData.get("priorityRationale") ?? "",
  ).trim();
  ...
  if (
    !CALENDAR_STATUS_VALUES.includes(
      calendarStatus as (typeof CALENDAR_STATUS_VALUES)[number],
    )
  ) {
  ...
  if (
    (calendarStatus === "active" || calendarStatus === "complete") &&
    !ownerId
  ) {
    return {
      error:
  ...
  if (decisionRaw) {
    if (
      !DECISION_VALUES.includes(decisionRaw as (typeof DECISION_VALUES)[number])
    ) {
