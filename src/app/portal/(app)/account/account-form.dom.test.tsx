...
describe("AccountForm", () => {
  ...
  test("Save is disabled until the preferred name changes", async () => {
    ...
    render(<AccountForm preferredName="Ave" fallbackName="Avery Morgan" />);
  ...
  test("falls back to the account name as the placeholder", () => {
    render(<AccountForm preferredName={null} fallbackName="Avery Morgan" />);
  ...
  test("submits the new preferred name and confirms", async () => {
    ...
    expect(
      await screen.findByText("Preferred name saved."),
    ).toBeInTheDocument();
  });
  ...
  test("surfaces a server error and does not claim success", async () => {
    ...
    expect(screen.queryByText("Preferred name saved.")).not.toBeInTheDocument();
  });
});
