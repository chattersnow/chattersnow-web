...
import { DESCRIPTION, TermsDocument } from "./document";

// The platform's own document renders unless the tenant has published its
// own under `legal.terms` (#707 Phase 4), in which case that replaces the page
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const doc = site.content.document("legal.terms");
  return {
...
export default async function Page() {
  ...
  const doc = content.document("legal.terms");
  return doc ? <LegalDocument doc={doc} /> : <TermsDocument />;
}
