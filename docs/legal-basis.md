...
- **Photo and media consent** (#599) — the same shape, and the first one where
  the "no" is worth as much as the "yes". An unticked box beneath the
  organization's own `events.photo_consent` scope, shown in full at the point of
  registration; `photo_consent`, `photo_consent_at` and `photo_consent_text`
  record the answer, the moment, and a copy of what was asked. Three things
  separate it from the waiver above:
  - **Three states, not two.** A waiver has two because declining it is
    declining to register, so no row survives a refusal. Here `false` is a
    stored decline with an operational job — it is what an organizer checks
    before pointing a camera — and `null` means nobody asked. Nothing may read
    `null` as either answer.
  - **A snapshot, not a version pointer.** #1319's test: the version-table
    shape earns its cost only where the text has to be citable from outside the
    row that accepted it. `/waiver?version=N` is a permalink; a content slot has
    no version table and no address, so the words are copied onto the row —
    from the tenant's own row inside the RPC, never from the client.
  - **It can be withdrawn.** `set_my_photo_consent()` lets the registrant flip
    the answer in either direction from `/my/registration/[id]`, re-stamping the
    date and re-snapshotting the scope. A consent that cannot be withdrawn is
    not consent; an acceptance, by contrast, records an act that happened and
    stands.

  The platform writes no scope of its own, and a tenant that has written none
  asks nothing — no box, no columns, a form byte-identical to the one before
  this shipped. What an organization does with a photo of somebody's face is
  off-platform and unknowable from this repository, so a default here would be
  rule 2's worst case: a commitment made on a tenant's behalf that a registrant
  then consented to.
...
**The same question about photo consent is resolved rather than carried, and
resolved the other way (#599).** `20260922080000` nulls all three columns beside
the name, the email and the `person_id`. Both are records of something somebody
said, so what separates them is what they are about: an acceptance is a fact
about an **act**, and it still means something once the name has gone, which is
why #686 kept it. Photo consent is a fact about a person's **face**. An
anonymized "declined" protects nobody, because there is no name left to check a
photograph against, and an anonymized "granted" authorizes nothing — what would
survive is a three-year-old permission attached to a row nobody can identify,
which is a liability rather than a record. Nothing had asserted that the waiver
pair survives at all until this ticket; `retention.integration.test.ts` now
asserts both halves in one test, since one rule doing opposite things is the
