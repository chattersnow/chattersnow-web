...
- Still owed: per-tenant retention policies, custom domains on Vercel, per-tenant branding and a content model for the public site, tenant provisioning, a UI for support grants, and per-tenant export/deletion (Phase 4); whether the users screen deactivates a platform-wide account or removes a membership is also a Phase 4 decision. Local development has no `custom_domain` on the Chatter Snow tenant, so it resolves through the sole-active-tenant fallback exactly as before.
