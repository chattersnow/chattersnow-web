...
- Still owed: per-tenant retention policies (Phase 5b) and the demo tenant (Phase 5d). Local development has no `custom_domain` on the Chatter Snow tenant, so it resolves through the sole-active-tenant fallback exactly as before.
