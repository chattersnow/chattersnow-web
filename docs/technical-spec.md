...
A host is also an **install identity**. The portal is a PWA (#1083): `/manifest.webmanifest` is a metadata route resolved per request, so each host's manifest carries that host's tenant — its `tenants.name`, its `brand.primary_deep` and `brand.background`, and an icon from `/api/app-icon/<size>` that composes the tenant's `brand.app_icon_url` (or, unset, its initials on its own colour) inside the maskable safe zone. Nothing on anyone's home screen names the platform, and because a PWA install is keyed to origin plus `scope`, two tenants on two hosts can never collide. `start_url` and `scope` are host-aware for the same reason the portal's URLs are: `/home` and `/` on a `portal.` host, `/portal/home` and `/portal` everywhere else. An unresolved host gets a manifest naming no organization, matching the root-metadata rule in §7. The service worker (`public/sw.js`, registered only over the portal's scope, never under automation or in development) caches `/_next/static/*` and a tenant-neutral `/offline.html` and nothing else — no document, no RSC payload, no `/api` response — so an installed app can never show a stale total or a signed-out shell; offline is an honest banner and a refused submit, not a queued write.
