...
Two further routes exist for the **platform's own** public site (#1328): **`/nonprofits`** and **`/business`**, the two audience paths, one implementation (`src/app/(public)/audience-page.tsx`) behind two URLs. They describe what the platform does in each audience's vocabulary — governance leads the nonprofit page and is deliberately absent from the business one — and both say plainly that the wording is a per-tenant setting rather than a separate edition. They are the one pair of routes in the tree whose content is about the product rather than about an organization, so their copy is the registry default rather than a prompt, and they sit behind a single `audiences` page-visibility slot that is **off for every tenant** until somebody turns it on. #1325 records the exit criterion: when the routes that exist only for the product's own marketing grow well past a handful, they move to a project of their own.
