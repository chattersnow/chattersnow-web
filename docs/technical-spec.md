...
- Phase 4 — serving more than one tenant (operator runbook: `docs/tenants.md`):
  ...
  - Isolation: `site_content`, `article_categories` and `articles` are in `TENANT_TABLES` and the suite; `src/lib/portal/tenant-provisioning.integration.test.ts` provisions a tenant, signs its admin in through the staged grant, exercises support access, removal, deactivation scoping, host-resolved content and branding, export and deletion.
