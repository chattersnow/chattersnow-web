...
- `current_tenant_id()`: the user's selection when backed by a live membership, else their sole membership, else null. Every session-scoped read uses it. `set_current_tenant()` is the only write path; `ensure_tenant_membership()` joins a first-time account to the sole active tenant and refuses to guess on a multi-tenant database.
- `default_tenant_id()` — Phase 2: the column default for `tenant_id`. `current_tenant_id()` when there is a session, else the sole active tenant, else null — so the anon intake RPCs, `pg_cron`, `supabase/seed.sql` and service-role fixtures keep working on a single-tenant database and fail closed (a `not null` violation) on a multi-tenant one. Phase 3 replaces the fallback with resolution from the request host; callers do not change. The eight anon `public_*` views filter on it.
...
- Still owed by Phase 3: the bulk rewrite of the remaining policies to `tenant_id = (select current_tenant_id()) and has_permission(...)`, host → tenant resolution for anon callers, the composite `(tenant_id, id)` foreign keys on every parent table, the audit of the remaining `security definer` report RPCs, and the generated cross-tenant isolation suite.
...
3. Authenticated users should receive only the permissions associated with their roles.
   - **Tenant isolation** (#707): every tenant table carries `tenant_id`, permissions are evaluated per tenant, and platform access is a time-boxed `support` membership rather than a bypass. Row-level enforcement across all tables and the generated isolation suite land in Phase 3; a gap there is a release blocker for serving a second organization.
