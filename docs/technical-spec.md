...
**Also implemented, on `events` itself:** an event lead (`event_lead_id` → `auth.users`), venue, capacity, and an after-phase report workflow (`report_status`: not_started/in_progress/submitted, `report_summary`, `lessons_learned`, `feedback_notes`, `content_notes`, `report_submitted_at`), surfaced on the event editor's Report tab. Event type and venue are set on the event editor's Overview card and on the create dialog, and are the two `events` fields the public event pages render (`event_type` as the eyebrow above the title, `venue` falling back to `location` as the sub-line). `event_type` draws on a curated list in `src/lib/event-types.ts` that the **UI** offers; the column itself is unconstrained `text` on purpose, because rows predating the list hold free text typed into the create dialog — pickers keep an unrecognised current value selectable and `eventTypeLabel` humanises it rather than blanking it, so adding a check constraint would break both.
...
Role membership itself follows the same principle: it is **derived, not stored**. `public.people_with_roles` (issue #624) is a `security_invoker` view carrying every `people` column plus `is_donor` / `is_sponsor` / `is_volunteer` / `is_attendee` / `is_staff`, each answered at read time by the `security definer` helper `person_role_flags()` from the records that create the role — donations, monetary donations, giveaway prizes, event sponsors, event registrations, event volunteers, volunteer hours, volunteer applications, event staff — unioned with `person_role_tags`. The helper is definer so a role never depends on the reader's access to the evidence behind it: an event coordinator holds `people:view` and `finance:none` and must still see a donor as a donor. The view is invoker so who may see the person is still decided by the `people` select policy. Reads use the view; every write still goes to `people`, and `person_role_tags` — a staff assertion with a date and an author — is the only place a role is ever written by hand.
