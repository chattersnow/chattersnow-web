...
- Still owed: the demo tenant (Phase 5d), and the platform operator UI (Phase 5c). Local development has no `custom_domain` on the Chatter Snow tenant, so it resolves through the sole-active-tenant fallback exactly as before.
