...
Data model, portal CRUD, the content-opportunity workflow, and audit history implemented (issues #103, #104, #106, #109); public surface, templates, work queues, and program intelligence remain planned — see §5.20 and issue #102.
...
- Audit coverage extends the existing `audit_log` pattern (§5.11) to `calendar_items` (issue #103) and `content_opportunities` (issue #109) rather than introducing a parallel history mechanism; both are wired via the generic `audit_log_row()` trigger, and the Administration > Audit log UI's table filter/labels cover both.
