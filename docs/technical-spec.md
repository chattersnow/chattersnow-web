...
Meetings, agendas, minutes, action items, decisions, board members, and nonprofit-status tracking are implemented; bylaws and policies remain placeholder pages with no backing tables yet.
...
- `conflict_of_interest_disclosures`: linked to a `people` record, disclosure year, on-file date, notes
- `annual_requirements`: name, due date, status (not_started/in_progress/done), completed-at, responsible `people` record
...
src/app/
  ...
  portal/
    ...
    (app)/                      # authenticated portal shell (sidebar layout)
      ...
      governance/               # board-members, meetings (incl. agendas), resolutions, conflict-of-interest, annual-requirements, nonprofit-status — implemented; bylaws, policies — placeholder
