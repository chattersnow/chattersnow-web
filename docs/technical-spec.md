...
- Phase 4 — serving more than one tenant (operator runbook: `docs/tenants.md`):
  ...
  - Content model: `site_content (tenant_id, key, value jsonb)` under a new `site_content` resource (admin manage), read by the site through `public_site_content` (host). The slot registry with Chatter Snow's copy as every default is `src/lib/site-content.ts` (text, paragraphs, typed lists, and a structured `document` per legal page that replaces the platform's own); `getPublicSite()` folds a tenant's rows over the defaults once per request. Edited page by page at Administration > Site Content. The Learn guides, sizing tables and form chrome stay in code.
...
- Still owed: per-tenant retention policies (the purge is one platform sweep and any tenant's admin can run or reconfigure it); the demo tenant (Phase 5). Local development has no `custom_domain` on the Chatter Snow tenant, so it resolves through the sole-active-tenant fallback exactly as before.
