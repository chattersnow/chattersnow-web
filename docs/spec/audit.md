...
- Currently audited: `donations`, `inventory_items`, `inventory_movements`, `event_expenses`, `user_roles`, `app_settings`, `calendar_items`, `pending_role_grants`, `content_opportunities`, `deactivated_users` (`pk_column = 'user_id'`), `event_revenue`, `reimbursements`, `person_claims` ([§5.23](constituent-accounts.md#523-constituent-accounts)), `volunteer_hour_submissions` ([§5.17](volunteers.md#517-volunteer-management)), `volunteer_screening_tiers` and `person_screenings` ([§5.17](volunteers.md#517-volunteer-management) — both with an empty `redacted_columns`, because neither table has a free-text column to redact), `public_team_members` ([§5.9](people.md#59-people-directory)), and the full giveaway set — `giveaways`, `giveaway_prizes`, `giveaway_winners`, `giveaway_tiers`, `giveaway_tier_grants`, `giveaway_tier_rules`, `giveaway_ticket_packages`, `giveaway_ticket_sales`, `giveaway_ticket_grants`, `giveaway_buckets` (issue #5: a ticket system that decides who wins gear needs a change history). Not yet covered: `events`
- `people` is the one entry in `audited_tables` with **no trigger** (issue #1164). It is registered so that `audit_log.table_name`'s foreign key resolves and the constituent self-edit functions (`set_my_contact_details()`, `confirm_email_change()`, [§5.9](people.md#59-people-directory), in the area specified in [§5.23](constituent-accounts.md#523-constituent-accounts)) can attribute a person's own change to that person; those two functions write their own rows. That is a deliberate exception to the trigger rule above, and it is narrow: there is exactly one self-edit write path by construction, while a trigger on `people` would audit every import, every registration upsert and every notification-token write — reversing #18's decision to leave the PII table out of the audited set, which is a platform decision rather than one ticket's. The snapshot those rows carry is the self-edit allowlist plus name and email, not the whole row, and every key in it is registered in `retention_snapshot_personal_columns`, so the `audit_log_snapshots` rule clears it on the same clock as a trigger's snapshots
