...
- `people.notification_email`: **implemented** (issue #1042) — where a portal user's email is delivered when that is not the address they sign in with. Sign-in binds an account to a directory record on `lower(people.email)` and that column is unique per tenant, so it is an identity key rather than a mailbox; somebody signing in with a personal Google account sets an override instead of editing what identifies them. Delivery is `coalesce(notification_email, email)` — returned by `people_with_permission()` in its `email` column for the sessionless senders, and resolved by `deliveryAddress()` (`src/lib/notifications/delivery-address.ts`) everywhere else. Set by the person at `/portal/account` (`set_my_notification_email`, `security definer` for the reason `set_my_preferred_name` is) or by `people:manage` on the Portal account card of a person's record (`set_notification_email_for_person`). Cleared with the rest of a person's contact details when they are anonymized, by the `normalize_person_email` trigger rather than by the retention function's column list.
