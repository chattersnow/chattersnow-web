...
Keep cards 2–4 inside **one `<form>` with one save**. `saveMyContactDetailsAction` writes the whole allowlist, so a per-card partial payload would clobber unsent columns unless the action learns field presence first. Pinning it is not the hard part: `details/contact-form.tsx:570-575` already ships a sticky bar, and the public event registration forms pin their submit directly (#1375). Keep the public convention — submit is the last child of the `FieldGroup`, left-aligned `w-full sm:w-fit`, and sticky on the button itself rather than on a wrapper, which is what gives it the tall containing block it needs to travel. What makes one save survivable at the foot of a long page is dirty-gating it the way `/portal/account`'s `AccountForm` does, so the button is never a live control you scrolled to for nothing. That gating is the difference from a public submit, which is always live because there is nothing yet to be dirty.
