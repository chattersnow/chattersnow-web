...
**Status.** All open. Findings 2 and 9 overlap work already in flight on #1175 and are marked so; nothing else has an owner.
...
| --- | ---------------------------------------------------------------------------------- | ----------- | -------- | -------------------- |
| 1   | No `PageShell` on any of the five routes: no padding, no column, no `<main>`       | Layout/a11y | Serious  | Open                 |
| 2   | Four destinations, no navigation surface — a `<p>` of buttons and three back links | IA          | Serious  | Open (part of #1175) |
| 3   | The area is excluded from every automated scan and has no e2e spec                 | Test infra  | Serious  | Open (part of #1175) |
...
| 9   | Sign-out is a button in the page body, not in chrome                               | IA          | Moderate | Open (#1175 owns)    |
| 10  | Required markers typed into label strings, defeating `FieldLabel required`         | a11y        | Minor    | Open                 |
| 11  | No `loading.tsx`/`error.tsx`; sign-in's Suspense fallback is `null`                | Perf/feel   | Minor    | Open                 |
| 12  | Pending affordances differ between the area's forms                                | Consistency | Minor    | Open                 |
| 13  | No `robots: { index: false }` on a signed-in private area                          | Semantics   | Minor    | Open                 |
